...
function renderEntries(filteredEntries = entries) {
    ...
    filteredEntries.forEach(entry => {
        ...
        row.innerHTML = `
            <td class="px-6 py-5 whitespace-nowrap text-sm font-medium text-slate-400">${formatDateBR(entry.date)}</td>
            <td class="px-6 py-5">
                <div class="text-sm font-semibold text-slate-100">${entry.desc}</div>
                <div class="text-xs text-slate-500">Categoria Pessoal</div>
            </td>
            ...
            <td class="px-6 py-5 whitespace-nowrap text-center actions-cell">
                <div class="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onclick="editEntry(${entry.id})" class="p-2 text-slate-400 hover:text-accent transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                    </button>
                    <button onclick="deleteEntry(${entry.id})" class="p-2 text-slate-400 hover:text-danger transition-colors">
